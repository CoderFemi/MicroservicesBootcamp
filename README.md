...
```
The test environment runs separately from other environments, therefore, it is necessary to mock some libraries for use. Jest provides the `mock()` method, which takes the path to the original library as an argument. The mocked library is located in the `__mocked__` folder with the exact same name as the original one. When a test depending on that library runs, it uses the mocked one instead of the original. Mocking a library involves identifying which of its properties or method is being executed in the app and mocking its implementation. Such properties/methods should be provided as arguments to `jest.fn().mockImplementation()` to enable us make test assertions about the jest mock function.
...
### Event Publishing Technique
There is always the possibility that a record is saved to the database, but not published to the event bus successfully. If this happens, other services will not be notifed of this event, and may lead to inaccuracy in the overall system records. Take for example, a banking transaction in which a user deposits some money. This transaction is saved to the database, and an event published to the accounts service to update the user's balance with the amount deposited. If this event fails, the user's account will not be updated, yet the transaction displays to the user as successful. To solve this, databases have a `transaction feature` that handles both the transaction and the published event mutually. If one of them fails, the transaction will be rolled back. If both succeed, only then will the transaction be recorded as successful.
